<nav class="navbar navbar-light navbar-expand-lg">
    <div class="container-fluid">
        <a class="navbar-brand" href="/">{{ __('msg.navbar-brand') }}</a>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarBurger" aria-controls="navbarBurger" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarBurger">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <li class="nav-item">
                    <a class="nav-link {{ Request::is('/') || Request::is('translate') ? 'active' : '' }}" aria-current="page" href="/">{{ __('msg.navlink-translate') }}</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ Request::is('imprint') ? 'active' : '' }}" aria-current="page" href="/imprint">{{ __('msg.navlink-imprint') }}</a>
                </li>
            </ul>
        </div>
    </div>
</nav>
<hr class="header-end" />
